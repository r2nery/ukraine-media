        ...
        with alive_bar(len(self.unique_urls), title="-> Reuters: Scraper", spinner="dots_waves", bar="smooth", force_tty=True) as bar:
            ...
            for url in self.unique_urls:
                try:
                    ...
                    title_selector = ["#__next > div > div > div > div > div > h1"]
                    ...
                    text_selector = ["#__next > div > div > div > article"]
                    html_text = session.get(url).text
                    soup = BeautifulSoup(html_text, "lxml")
                    title = soup.select_one("h1", class_=title_selector).text
                    date = soup.find_all("span", class_=date_tags)
                    date = date[1].text
                    body = soup.select_one("p", class_=text_selector).text
                    # body = ""
                    # for _ in paragraphs:
                    #     body += " " + _.text
